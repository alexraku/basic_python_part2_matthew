...
def one_year_of_war():
    ...
    # Вы можете изменять состав команды, НО размер
    # команды не должен быть более 5.
    ...
    # Если умирают герои - цикл завершается - битва
    # считается проигранной (возвращается 0)
    # Если герои выживают - битва считается выигранной (возвращается 1)
    if sum([isinstance(hero, (MonsterHunter, MonsterBerserk))
            for hero in good_team]) > 1:
    ...
    for day in range(1, 366):
        ...
        # В циклах у героев и монстров вызывается метод make_a_move, который
        # должен выбирать и совершать одно действие
        # Для наглядности вы можете добавлять в каждое действие принты с
        # подробностями (чтобы знать кто когда и что совершает)
        # При помощи этой информации вы сможете искать проблемы и ошибки
        # в вашем коде и в конечном итоге это поможет вам улучшить стратегию
        print("\nКоманда добра:\n" + '-' * 50)
        ...
        # В итогах дня у каждого героя и каждого монстра вызывается метод
        # __str__ который должен описывать их текущее состояние
        print("\nКоманда добра:\n" + '-' * 50)
        for hero in good_team:
            print(hero)
        ...
        # Новые монстры в чётные дни добавляются в список
        # (но их не может быть больше 4)
        if day % 2 == 0 and len(evil_team) < 4:
            newborn_evils = [MonsterBerserk("Берсерк " +
                             random.choice(evil_names)),
                             MonsterHunter("Рейнджер " +
                             random.choice(evil_names))]
...
# Он запускает 20 симуляций. Для зачёта по заданию вам надо
# стабильно набирать 10 или более побед.
count_of_wins = 0
